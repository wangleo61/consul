...
A `task.condition` block optionally configures the condition that needs to be met in order to trigger a task. When not configured, the default condition is to trigger the task on changes in services configured in `task.services`.

#### Catalog Service Condition

A Catalog Service condition block configures a task to only trigger on service registration and deregistration, more specifically on first service instance registration and last service instance deregistration respectively. The specific services that will trigger the task on registration and deregistration can be configured within the condition block.
...
- `datacenter` - (string) The datacenter of the services to query for the task. If not provided, defaults to the datacenter of the agent that CTS queries.
...
- `node_meta` - (map[string]) The node metadata key/value pairs with which to filter services such that only services registered at a node with the specified key/value pairs are used by the task.
- `regexp` - (string) Optional only if [`task.services`](/docs/nia/configuration#services) is configured. Either `regexp` or `task.services` must be configured, or both. The regular expression matches on service names. Matched services will trigger the task to run when the services registers or deregisters. If not provided, defaults to an exact match on `task.services` list. For example, if `task.services = ["api", "web"]`, then regexp will default to `^api$|^web$`. Please see the following resources for more information: [regular expression syntax](https://github.com/google/re2/wiki/Syntax), [regular expression test playground](https://golang.org/pkg/regexp/#Regexp.MatchString).
- `source_includes_var` - (bool: false) Configure depending on whether the module configured at [`task.source`](#source) includes the `catalog_services` variable. Please refer to the documentation of the selected module for guidance on how this field should be configured.
