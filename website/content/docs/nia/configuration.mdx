...
- `buffer_period` - Configures the buffer period for the task to dampen the affects of flapping services to downstream network devices. It defines the minimum and maximum amount of time to wait for the cluster to reach a consistent state and accumulate changes before triggering task execution. The default is inherited from the top level [`buffer_period` block](#global-config-options). If configured, these values will take precedence over the global buffer period. This is useful to enable for a task that is dependent on services that have a lot of flapping.
  ...
  - `min` - (string: "5s") The minimum period of time to wait after changes are detected before triggering related tasks.
  - `max` - (string: "20s") The maximum period of time to wait after changes are detected before triggering related tasks. If `min` is set, the default period for `max` is 4 times the value of `min`.
- `condition` - (obj) The requirement that when met, triggers CTS to run the task. When not configured, the default condition is to trigger the task on changes in the services listed in `services`. [Task condition configuration](#task-condition) options are described below.

### Task Condition

A `task.condition` block optionally configures the condition that needs to be met in order to trigger a task. Below are the different types of conditions.
#### Services Condition

This is the default type of condition when no `task.condition` block is configured. This condition will trigger a task on changes in services configured in `task.services`. The task can be optionally configured with services condition block e.g. `condition services {}`; however this is not necessary since this is the default type of condition. Currently there are no additional configuration options for the services condition.
...
task {
  ...
  // optional: declare an empty services block
  condition "services" {
...
A catalog-services condition block configures a task to only trigger on service registration and deregistration, more specifically on first service instance registration and last service instance deregistration respectively. The specific services that will trigger the task on registration and deregistration can be configured within the condition block.
...
task {
  ...
  // optionally provides additional information to module. refer to module docs
  // on whether or not to configure `services`. does not trigger task
  services = ["web-api"]
...
- `datacenter` - (string) The datacenter of the services to query for the task. If not provided, the datacenter will default to the datacenter of the agent that CTS queries.
...
- `node_meta` - (map[string]) The node metadata key/value pairs to filter services. Only services registered at a node with the specified key/value pairs are used by the task.
- `regexp` - (string) Optional if [`task.services`](/docs/nia/configuration#services) is configured. Either `regexp` or `task.services` must be configured, or both. The regular expression matches on service names. Matched services will trigger the task to run when the services registers or deregisters. If not provided, regexp will default to an exact match on `task.services` list. For example, if `task.services = ["api", "web"]`, then regexp will default to `^api$|^web$`. Please see the following resources for more information: [regular expression syntax](https://github.com/google/re2/wiki/Syntax), [test out regular expression MatchString](https://golang.org/pkg/regexp/#Regexp.MatchString).
- `source_includes_var` - (bool: false) Configure depending on whether the module configured at [`task.source`](#source) includes the [`catalog_services` variable](/docs/nia/terraform-modules#catalog-services-variable). Please refer to the documentation of the selected module for guidance on how this field should be configured. If configured inconsistent with the module, Consul-Terraform-Sync will error and exit.
